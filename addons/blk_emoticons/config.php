...
?>
<?php echo elementTitle('BLK Emoticons', 'loadLob(\'admin/setting_addons.php\');'); ?>
<style>
...
<div class="page_full">
	<div class="page_element">
		<div class="config_section">
			<p class="label">فئات الإيموجي (الباقات)</p>
			<div id="blk_categories_list" class="blk_emo_categories"></div>
		</div>
		<div class="config_section">
			<p class="label">إضافة فئة جديدة</p>
			<div class="setting_element">
				<input id="blk_new_cat_name" class="full_input" type="text" placeholder="اسم الفئة بالإنجليزي (مثال: custom1 أو sticker1)"/>
			</div>
			<div class="setting_element">
				<p class="ex_admin sub_text">أيقونة الفئة اللي تظهر بقائمة التبويبات (png / svg / gif) - اختياري</p>
				<input id="blk_new_cat_icon" type="file" accept=".png,.svg,.gif"/>
			</div>
			<button onclick="blkAddCategory();" type="button" class="tmargin10 reg_button theme_btn"><i class="fa fa-plus"></i> إضافة الفئة</button>
		</div>
	</div>
</div>
	...
	<div class="blk_emo_modal">
		...
		<div class="pad10">
			...
			<div class="setting_element tmargin10">
				...
				<button onclick="blkUploadFiles();" type="button" class="tmargin10 reg_button theme_btn"><i class="fa fa-upload"></i> رفع إيموجي</button>
			</div>
			<p class="ex_admin sub_text">اضغط على صورة الإيموجي لتعديل كوده (اسمه)، وعلى علامة X لحذفه.</p>
		</div>
	</div>
</div>
...
blkDeleteCategory = function(name){
	if(!confirm('حذف هذه الفئة سيحذف كل الإيموجيات اللي بداخلها, متأكد؟')){
		return;
	}
...
blkLoadFiles = function(){
	$.post('addons/blk_emoticons/system/action.php', { op: 'list_files', category: blkCurrentCat }, function(response){
		...
		if(data.files.length == 0){
			html = '<p class="ex_admin sub_text">لا توجد إيموجيات بهذه الفئة بعد.</p>';
		}
...
blkRenameFile = function(file, oldCode){
	var newCode = prompt('كود الإيموجي الجديد (بدون النقطتين):', oldCode);
